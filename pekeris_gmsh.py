...
# Physical tag constants
WATER_DOMAIN = 1
SEDIMENT_DOMAIN = 2
SOURCE_BOUNDARY = 10
TOP_BOUNDARY = 11       # Pressure-release surface (z=0)
BOTTOM_BOUNDARY = 12    # Truncation boundary (z=z_max)
RIGHT_BOUNDARY = 13     # Far-field boundary (r=r_max)
...
def gmsh_pekeris_waveguide(
    ...
    """Create a Gmsh model of a Pekeris waveguide cross-section.
    ...
    r_max : float
        Maximum radial extent of domain (m).
    z_max : float
        Maximum depth of domain (m), must be > H.
    r_source : float
    ...
    # Create water layer rectangle (from surface to interface)
    # Rectangle: corner at (0, 0), width r_max, height H
    water_rect = occ.addRectangle(0, 0, 0, r_max, H)

    # Create sediment layer rectangle (from interface to bottom)
    sediment_rect = occ.addRectangle(0, H, 0, r_max, z_max - H)

    # Create semicircular exclusion around source at (0, z_s)
    # We create a disk and then cut it to get a semicircle (r >= 0)
    ...
    # Fragment to create shared interface between water and sediment
    all_surfaces = [(2, water_tag), (2, sediment_rect)]
    fragmented = occ.fragment(all_surfaces, [])
    occ.synchronize()

    # Identify the resulting surfaces
    # After fragmentation, we need to determine which surface is water and which is sediment
    ...
    for dim, tag in surfaces:
        # Get the center of mass to determine which layer this surface belongs to
    ...
    for dim, tag in all_curves:
        ...
        if xmax <= r_source + tol and xmin < tol:
            ...
            # Get parametric bounds and sample points
        ...
        if abs(ymax - ymin) < tol:
            ...
            elif abs(y_val - H) < tol:  # z = H (interface)
                interface_curves.append(tag)
            elif abs(y_val - z_max) < tol:  # z = z_max (bottom)
        ...
        if abs(xmax - xmin) < tol:
            ...
            elif abs(x_val - r_max) < tol:  # r = r_max (right boundary)
...
if __name__ == "__main__":
    ...
    print(f"  Domain: r in [0, {args.r_max}] m, z in [0, {args.z_max}] m")
